...
```{r, warning=FALSE}
ratings %>%
  mutate(publicsystem = fct_relevel(publicsystem, 'CSU', 'SUNY', 'CUNY'),
  ...
  geom_text_repel(aes(data=(. %>% filter(publicsystem == 'CUNY')), label = institution)) +
  ...
  scale_colour_brewer(palette="Set1", na.value='grey50', name=NULL, labels=c("CSU", "SUNY", "CUNY", "Other")) +
  scale_alpha_discrete(range = c(1, 1), na.value=0.2, name=NULL, labels=c("CSU", "SUNY", "CUNY", "Other")) +
  scale_size_discrete(range = c(2,2), na.value=1, name=NULL, labels=c("CSU", "SUNY", "CUNY", "Other")) +
  geom_point() +
  labs(title='Scatterplot of access rate and success rate for all colleges') +
  ylab('Success rate') +
  xlab('Access rate') +
  theme_bw() + 
